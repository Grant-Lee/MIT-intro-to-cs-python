...
class Message(object):
    ...
    def build_shift_dict(self, shift):
        ...
        for i in range(26):
            if i + key < 26:
                self.shifted[lowercase[i]] = lowercase[i+key]
            else:
                self.shifted[lowercase[i]] = lowercase[i+key-26]
    
        for j in range(len(uppercase)):
            if j + key < len(uppercase):
                self.shifted[uppercase[j]] = uppercase[j+key]
            else:
                self.shifted[uppercase[j]] = uppercase[j+key-26]
        return self.shifted
...
class PlaintextMessage(Message):
    ...
    def get_encrypting_dict(self):
        ...
        '''
        self.temp_dict = self.build_shift_dict(self.shift).copy()
...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        best_shift = 0        
        best_bingo = 0
        while trial_shift < 26: 
            bingo = 0 
            for text in self.apply_shift(trial_shift).split(' '):
